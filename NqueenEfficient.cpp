...
bool nqueen(int col,vector<vector<int>> &mat,vector<int>  &rowhash,vector<int>  &thirdhash,vector<int>  &firsthash)
{
...
for(int row=0;row<n;row++)
{
   // fill row if it is safe(i.e its not attacking)
    if(issafe(row,col,mat,rowhash,thirdhash,firsthash)) // can i fill if it is safe
    {
        ...
        if(nqueen(col+1,mat,rowhash,thirdhash,firsthash)==true) // move to next col
            return true;
...
return false ;// unable to get safe place for my queen to be placed
}
int main()
{
